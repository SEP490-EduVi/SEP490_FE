...
export default function GameDemoPage() {
  ...
  return (
    <main className="min-h-screen bg-gray-950 text-white flex flex-col items-center py-10 px-4">
      <h1 className="text-3xl font-bold mb-2 text-white/90">🎮 Game Demo — Bàn phím</h1>
      <p className="text-sm text-white/50 mb-6">Demo blueprint game giáo dục, không cần camera</p>
      ...
      <div className="flex gap-4 mb-4 flex-wrap justify-center">
        ...
        {activeGame && (
          <button
            onClick={stopGame}
            className="px-5 py-3 rounded-xl bg-white/10 hover:bg-white/20 font-semibold transition"
          >
            ✕ Dừng
          </button>
        )}
      ...
      {status && <p className="text-xs text-white/40 mb-3">{status}</p>}
      ...
      >
        ...
        {!activeGame && (
          <div className="absolute inset-0 flex flex-col items-center justify-center gap-3 select-none">
            ...
            <p className="text-white/50 text-sm">Chọn một game ở trên để bắt đầu</p>
          </div>
        )}
      ...
      <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 gap-4 w-full max-w-3xl">
        <div className="bg-white/5 rounded-xl p-4 border border-white/10">
          <h3 className="font-semibold mb-2 text-red-400">🏃 Mario Runner Quiz (1P)</h3>
        ...
        <div className="bg-white/5 rounded-xl p-4 border border-white/10">
          <h3 className="font-semibold mb-2 text-emerald-400">🐍 Snake Quiz (1P)</h3>
          <ul className="text-sm text-white/55 space-y-1">
        ...
        <div className="bg-white/5 rounded-xl p-4 border border-orange-500/20">
          <h3 className="font-semibold mb-2 text-orange-400">🏃‍♂️🏃‍♀️ Runner Race (2P)</h3>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <p className="text-xs text-red-300 font-semibold mb-1">P1 (đỏ)</p>
              <ul className="text-sm text-white/55 space-y-0.5">
            ...
            <div>
              <p className="text-xs text-blue-300 font-semibold mb-1">P2 (xanh)</p>
              <ul className="text-sm text-white/55 space-y-0.5">
        ...
        <div className="bg-white/5 rounded-xl p-4 border border-cyan-500/20">
          <h3 className="font-semibold mb-2 text-cyan-400">🐍🐍 Snake Duel (2P)</h3>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <p className="text-xs text-green-300 font-semibold mb-1">P1 (xanh lá)</p>
              <ul className="text-sm text-white/55 space-y-0.5">
            ...
            <div>
              <p className="text-xs text-cyan-300 font-semibold mb-1">P2 (cyan)</p>
              <ul className="text-sm text-white/55 space-y-0.5">
...
function Kbd({ children }: { children: React.ReactNode }) {
  return (
    <kbd className="inline-block bg-white/10 border border-white/20 rounded px-1.5 py-0.5 text-xs font-mono mr-1">
      {children}
    </kbd>
  );
}
