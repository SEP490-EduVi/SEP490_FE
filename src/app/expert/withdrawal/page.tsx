...
export default function ExpertWithdrawalPage() {
  ...
  const [fields, setFields] = useState<Step1Fields>(EMPTY_STEP1);
  const [otp, setOtp] = useState('');
  const [lastWithdrawal, setLastWithdrawal] = useState<WithdrawalDto | null>(null);

  const amountNum = parseFloat(fields.amount.replace(/\D/g, '')) || 0;

  const handleInitiate = async (e: React.FormEvent) => {
    e.preventDefault();
    if (amountNum < 200_000) {
      notify.error('Số tiền rút tối thiểu là 200.000 ₫');
      return;
    }
    if (!fields.bankAccountNumber.trim() || !fields.bankName.trim() || !fields.accountHolderName.trim()) {
      notify.error('Vui lòng điền đầy đủ thông tin ngân hàng.');
      return;
    }
    try {
      await initiateMutation.mutateAsync({
  ...
  const handleConfirm = async (e: React.FormEvent) => {
    ...
    if (otp.trim().length !== 6) {
      notify.error('Vui lòng nhập mã OTP 6 chữ số.');
      return;
    }
    try {
      const result = await confirmMutation.mutateAsync({
      ...
      const msg = (err as { response?: { data?: { message?: string } } })?.response?.data?.message ?? 'OTP không hợp lệ hoặc đã hết hạn.';
      notify.error(msg);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-emerald-50/30">
      ...
      <div className="mx-auto max-w-5xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
          ...
          <div className="lg:col-span-1">
            ...
            <div className="mt-4 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
              <p className="text-xs font-semibold uppercase tracking-wide text-slate-400 mb-3">Các bước</p>
              {[
                { key: 'form', label: 'Nhập thông tin' },
                { key: 'otp',  label: 'Xác nhận OTP' },
                { key: 'done', label: 'Hoàn tất' },
                ...
                const isActive = step === s.key;
                const isDone = (step === 'otp' && i === 0) || (step === 'done' && i < 2);
                return (
                  <div key={s.key} className="flex items-center gap-3 py-1.5">
                    <div className={`flex h-6 w-6 items-center justify-center rounded-full text-xs font-bold transition-colors ${
                      isDone ? 'bg-emerald-100 text-emerald-600' : isActive ? 'bg-emerald-600 text-white' : 'bg-slate-100 text-slate-400'
                    }`}>
                      {isDone ? <CheckCircle2 className="h-4 w-4" /> : i + 1}
                    </div>
                    <span className={`text-sm ${isActive ? 'font-semibold text-slate-800' : 'text-slate-500'}`}>{s.label}</span>
              ...
              <AnimatePresence mode="wait">
                ...
                {step === 'form' && (
                  <motion.form
                    ...
                    exit={{ opacity: 0, x: -20 }}
                    onSubmit={(e) => void handleInitiate(e)}
                    className="space-y-5"
                  >
                    <h2 className="text-lg font-semibold text-slate-800">Thông tin rút tiền</h2>
                    ...
                    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                      ...
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) => setFields((f) => ({ ...f, bankAccountNumber: e.target.value }))}
                          className="w-full rounded-xl border border-slate-200 px-4 py-2.5 text-sm outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100"
                        />
                      </div>
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) => setFields((f) => ({ ...f, accountHolderName: e.target.value }))}
                          className="w-full rounded-xl border border-slate-200 px-4 py-2.5 text-sm outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100"
                        />
                      </div>
                      <div>
                        ...
                        />
                        {amountNum > 0 && (
                          <p className="mt-1 text-xs text-emerald-600 font-medium">{formatVND(amountNum)}</p>
                        )}
                      </div>
                    </div>

                    <div className="rounded-xl bg-amber-50 border border-amber-100 px-4 py-3 text-xs text-amber-700">
                      Sau khi gửi, một mã OTP sẽ được gửi đến email của bạn. Số tiền sẽ bị giữ tạm thời cho đến khi Admin duyệt.
                    </div>
                    ...
                    <button
                      ...
                      className="flex w-full items-center justify-center gap-2 rounded-xl bg-emerald-600 px-6 py-3 text-sm font-semibold text-white shadow-sm transition hover:bg-emerald-700 disabled:opacity-60"
                    >
                      {initiateMutation.isPending ? 'Đang gửi...' : (
                ...
                {step === 'otp' && (
                  ...
                  >
                    ...
                    <div className="rounded-xl bg-slate-50 border border-slate-200 px-4 py-3 text-sm text-slate-600 space-y-1">
                      <p><span className="font-medium">Ngân hàng:</span> {fields.bankName}</p>
                      <p><span className="font-medium">Số TK:</span> {fields.bankAccountNumber} — {fields.accountHolderName}</p>
                      <p><span className="font-medium">Số tiền:</span> <span className="font-semibold text-emerald-600">{formatVND(amountNum)}</span></p>
                    </div>
                    ...
                    <div>
                      ...
                      <input
                        ...
                        value={otp}
                        onChange={(e) => setOtp(e.target.value.replace(/\D/g, '').slice(0, 6))}
                        className="w-full rounded-xl border border-slate-200 px-4 py-3 text-center text-2xl font-bold tracking-widest outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100"
                      />
                      <p className="mt-1.5 text-xs text-slate-400">OTP được gửi đến email của bạn và có hiệu lực trong 5 phút.</p>
                    </div>
                    ...
                    <button
                      ...
                      className="flex w-full items-center justify-center gap-2 rounded-xl bg-emerald-600 px-6 py-3 text-sm font-semibold text-white shadow-sm transition hover:bg-emerald-700 disabled:opacity-60"
                    >
                      {confirmMutation.isPending ? 'Đang xác nhận...' : (
                        <>Xác nhận rút tiền <CheckCircle2 className="h-4 w-4" /></>
                      )}
                    </button>
                  </motion.form>
        ...
        {/* History */}
        <div className="mt-8">
          <h2 className="mb-4 text-lg font-semibold text-slate-800">Lịch sử rút tiền</h2>
          <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
            {historyLoading ? (
              <p className="px-6 py-12 text-center text-sm text-slate-400">Đang tải...</p>
            ) : withdrawals.length === 0 ? (
              <p className="px-6 py-12 text-center text-sm text-slate-400">Chưa có yêu cầu rút tiền nào.</p>
            ) : (
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-slate-100 bg-slate-50/70">
                    <th className="px-5 py-3 text-left font-medium text-slate-500">Số tiền</th>
                    <th className="px-5 py-3 text-left font-medium text-slate-500">Ngân hàng</th>
                    <th className="px-5 py-3 text-left font-medium text-slate-500">Trạng thái</th>
                    <th className="px-5 py-3 text-left font-medium text-slate-500">Thời gian</th>
                    <th className="px-5 py-3 text-left font-medium text-slate-500">Ghi chú Admin</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100">
                  {withdrawals.map((w) => {
                    ...
                    return (
                      <tr key={w.withdrawalId} className="hover:bg-slate-50">
                        ...
                        <td className="px-5 py-3 text-xs text-slate-500">{w.adminNote ?? '—'}</td>
                      </tr>
                    );
                  })}
                </tbody>
