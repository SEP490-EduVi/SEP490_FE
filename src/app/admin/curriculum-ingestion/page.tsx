...
import { BookOpen, Eye, FileText, Loader2, RefreshCw, Trash2, Upload, X } from 'lucide-react';
...
function StatusBadge({ doc }: { doc: CurriculumDto }) {
  ...
  return (
    <div
      ...
      className={`cursor-pointer select-none rounded-xl border-2 border-dashed p-5 text-center transition-all ${
        file ? 'border-indigo-300 bg-indigo-50' : 'border-gray-200 hover:border-indigo-300 hover:bg-indigo-50/40'
      }`}
    >
      {file ? (
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0 flex-1 text-left">
            <p className="truncate text-sm font-medium text-indigo-700">{file.name}</p>
...
const EMPTY_FORM = { SubjectCode: '', EducationLevel: '', CurriculumYear: '', Note: '' };
const labelInput = 'block text-xs font-semibold uppercase tracking-wide text-gray-500 mb-1.5';
const baseInput = 'w-full rounded-xl border border-gray-200 bg-gray-50 px-3 py-2.5 text-sm text-gray-900 placeholder-gray-400 transition-colors focus:border-indigo-400 focus:bg-white focus:outline-none focus:ring-2 focus:ring-indigo-100';

export default function CurriculumIngestionPage() {
  const [docs, setDocs] = useState<CurriculumDto[]>([]);
  ...
  const [detailDoc, setDetailDoc] = useState<CurriculumDto | null>(null);
  const pollingRef = useRef<ReturnType<typeof setInterval> | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<CurriculumDto | null>(null);
  ...
  const loadList = useCallback(async () => {
    ...
    try { setDocs(await getCurricula()); }
  ...
  const startPolling = (documentCode: string) => {
    ...
    pollingRef.current = setInterval(async () => {
      try {
        const updated = await getCurriculumByCode(documentCode);
  ...
  const openDetail = (doc: CurriculumDto) => {
    setDetailDoc(doc);
  ...
  const handleUpload = async () => {
    if (!form.SubjectCode.trim() || !form.EducationLevel.trim() || !form.CurriculumYear.trim()) {
      notify.error('Vui lòng nhập đầy đủ Mã môn, Cấp học và Năm chương trình.'); return;
    }
    if (!file) { notify.error('Vui lòng chọn file .docx.'); return; }
    setBusy(true);
    try {
      const newDoc = await uploadCurriculum({
        File: file,
  ...
  const handleDeleteNeo4j = async () => {
    ...
    setBusy(true);
    try {
      await deleteCurriculumNeo4j(deleteTarget.documentCode);
    ...
    <div className="min-h-full bg-gray-50/60 p-6 space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="mt-0.5 text-sm text-gray-500">Upload và theo dõi xử lý chương trình giáo dục (.docx)</p>
        </div>
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => setUploadOpen(true)}
            ...
            <Upload className="h-4 w-4" /> Upload chương trình
          </button>
        </div>
      </div>
      ...
      <div className="rounded-2xl border border-gray-200 bg-white shadow-sm">
        <div className="border-b border-gray-100 px-5 py-4">
          <p className="text-sm font-semibold text-gray-700">
            Danh sách tài liệu
            <span className="ml-2 rounded-full bg-indigo-50 px-2 py-0.5 text-xs font-bold text-indigo-600">{docs.length}</span>
          </p>
        </div>
          ...
          <table className="min-w-full divide-y divide-gray-100 text-sm">
            <thead>
              <tr className="bg-gray-50/70">
                {['Mã tài liệu', 'Tên file', 'Mã môn', 'Cấp học', 'Năm CT', 'Trạng thái', 'Ghi chú', ''].map((h) => (
            ...
            <tbody className="divide-y divide-gray-50">
              {loading ? (
                ...
                <tr><td colSpan={8} className="py-16 text-center">
                  ...
                  <p className="mt-2 text-sm text-gray-400">Chưa có chương trình nào được upload.</p>
                </td></tr>
              ) : docs.map((doc) => (
                <tr key={doc.documentCode} className="hover:bg-indigo-50/30 transition-colors">
                  <td className="px-5 py-3.5 font-mono text-xs text-indigo-600">{doc.documentCode}</td>
                  ...
                  <td className="px-5 py-3.5 text-gray-600">{doc.educationLevel}</td>
                  <td className="px-5 py-3.5 text-gray-600">{doc.curriculumYear}</td>
                  <td className="px-5 py-3.5"><StatusBadge doc={doc} /></td>
                  <td className="max-w-[160px] px-5 py-3.5 text-xs text-gray-400 truncate">{doc.note ?? '—'}</td>
                  <td className="px-5 py-3.5">
                    <div className="flex items-center gap-1.5">
                      <button onClick={() => openDetail(doc)}
                        className="flex items-center gap-1 rounded-lg border border-gray-200 bg-white px-2.5 py-1.5 text-xs font-medium text-gray-600 shadow-sm hover:border-indigo-300 hover:text-indigo-600 transition-colors">
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.96, y: 8 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.96, y: 8 }}
              ...
              <div className="flex items-center justify-between border-b border-gray-100 px-6 py-4">
                <h2 className="text-base font-bold text-gray-900">Upload chương trình giáo dục</h2>
              ...
              <div className="space-y-4 p-6">
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    <label className={labelInput}>Cấp học *</label>
                    <input type="text" className={baseInput} placeholder="VD: THPT" maxLength={20}
                      value={form.EducationLevel} onChange={(e) => setForm((f) => ({ ...f, EducationLevel: e.target.value }))} />
                  </div>
                </div>
                <div>
                  <label className={labelInput}>Năm chương trình *</label>
                  <input type="number" className={baseInput} placeholder="VD: 2024"
                    value={form.CurriculumYear} onChange={(e) => setForm((f) => ({ ...f, CurriculumYear: e.target.value }))} />
                </div>
                <div>
                ...
                <div>
                  <label className={labelInput}>File chương trình (.docx) *</label>
                  <FileDropZone file={file} accept=".docx,application/vnd.openxmlformats-officedocument.wordprocessingml.document"
                    hint="Chỉ chấp nhận file .docx" onChange={setFile} onClear={() => setFile(null)} />
                </div>
              </div>
              <div className="flex justify-end gap-3 border-t border-gray-100 px-6 py-4">
                ...
                <button disabled={busy} onClick={() => void handleUpload()}
                  className="flex items-center gap-1.5 rounded-xl bg-indigo-600 px-5 py-2 text-sm font-semibold text-white hover:bg-indigo-700 disabled:opacity-50 transition-colors">
        ...
        {detailDoc && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.96, y: 8 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.96, y: 8 }}
              className="relative w-full max-w-lg rounded-2xl bg-white shadow-2xl">
              <div className="flex items-center justify-between border-b border-gray-100 px-6 py-4">
                <h2 className="text-base font-bold text-gray-900">Chi tiết chương trình</h2>
                <div className="flex items-center gap-2">
              ...
              <div className="p-6 space-y-3">
                <div className="grid grid-cols-2 gap-3">
                  {([
                    ...
                    ['Cấp học', detailDoc.educationLevel],
                    ['Năm CT', String(detailDoc.curriculumYear)],
                ...
                {detailDoc.note && (
                  <div className="rounded-xl bg-gray-50 p-3">
                    <p className="text-[10px] font-semibold uppercase tracking-wide text-gray-400 mb-0.5">Ghi chú</p>
                    <p className="text-sm text-gray-700">{detailDoc.note}</p>
                  </div>
                )}
              </div>
            </motion.div>
